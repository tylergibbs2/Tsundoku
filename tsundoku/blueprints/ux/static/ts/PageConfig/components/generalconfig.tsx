...
export const GeneralConfigApp = () => {
    ...
    const mutation = useMutation(async ({ key, value }: MutateConfigVars) => {
        ...
        {
            onSuccess: (newConfig) => { queryClient.setQueryData(["config", "general"], newConfig); }
        }
    );
    ...
    const inputUpdateCheck = (e: ChangeEvent<HTMLInputElement>) => {
        if (e.target.checked)
            ...
            mutation.mutate({ key: "update_do_check", value: false});
    }

    const inputLocale = (e: ChangeEvent<HTMLSelectElement>) => {
        mutation.mutate({ key: "locale", value: e.target.options[e.target.selectedIndex].value });
    }

    const inputLogLevel = (e: ChangeEvent<HTMLSelectElement>) => {
        mutation.mutate({ key: "log_level", value: e.target.options[e.target.selectedIndex].value });
    }
    ...
    const inputUnwatchWhenFinished = (e: ChangeEvent<HTMLInputElement>) => {
        if (e.target.checked)
            ...
            mutation.mutate({ key: "unwatch_when_finished", value: false});
    }
        ...
        <div className="box">
            <div className="columns is-multiline">
                <div className="column is-3">
                    ...
                    <div className="field has-addons">
                        ...
                        <div className="control">
                            <input className="input" type="number" placeholder="6439" min="1" max="65535" value={config?.data?.port} onChange={inputPort} />
                        </div>
                    </div>
                </div>
                <div className="column is-3">
                    ...
                    <div className="select is-fullwidth">
                        <select onChange={inputLogLevel} defaultValue={config?.data?.log_level}>
                ...
                <div className="column is-3">
                    ...
                    <div className="field">
                        <input id="updateCheck" type="checkbox" className="switch" checked={config?.data?.update_do_check} onChange={inputUpdateCheck} />
                ...
                <div className="column is-4">
                    ...
                    <input className="input" type="text" value={config?.data?.default_desired_format} onChange={inputDefaultDesiredFormat} />
                </div>
                <div className="column is-4">
                    ...
                    <input className="input" type="text" value={config?.data?.default_desired_folder} onChange={inputDefaultDesiredFolder} />
                </div>
                <div className="column is-4">
                    ...
                    <div className="field">
                        <input id="unwatchCheck" type="checkbox" className="switch" checked={config?.data?.unwatch_when_finished} onChange={inputUnwatchWhenFinished} />
