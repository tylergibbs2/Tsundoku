...
export const DeleteModal = ({ activeModal, setActiveModal, activeWebhook, setActiveWebhook }: DeleteModalParams) => {
    const queryClient = useQueryClient();

    const mutation = useMutation(deleteWebhookById,
        {
            onSuccess: () => {
                queryClient.setQueryData(["webhooks"], (oldWebhooks: WebhookBase[]) => oldWebhooks.filter((wh) => wh.base_id !== activeWebhook?.base_id));
                toast({
    ...
    return (
        <div className={"modal modal-fx-fadeInScale " + (activeWebhook && activeModal === "delete" ? "is-active" : "")}>
            ...
            <div className="modal-card">
                ...
                <section className="modal-card-body">
                    <p>{activeWebhook && ReactHtmlParser(_("delete-confirm-text", { "name": activeWebhook.name }))}</p>
                </section>
