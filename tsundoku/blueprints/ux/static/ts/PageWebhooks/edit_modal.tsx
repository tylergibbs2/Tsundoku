...
export const EditModal = ({ activeModal, setActiveModal, activeWebhook, setActiveWebhook }: EditModalParams) => {
    ...
    const mutation = useMutation(updateWebhookById,
        {
            onSuccess: (updatedWebhook) => {
                queryClient.setQueryData(["webhooks"], (oldWebhooks: WebhookBase[]) => [...oldWebhooks.filter((wh) => wh.base_id !== updatedWebhook.base_id), updatedWebhook]);
                toast({
    ...
    const submitHandler: SubmitHandler<EditWebhookFormValues> = (formData: EditWebhookFormValues) => {
        mutation.mutate({
    ...
    return (
        <div className={"modal modal-fx-fadeInScale " + (activeModal === "edit" ? "is-active" : "")}>
            ...
            <div className="modal-card">
                ...
                <footer className="modal-card-foot is-size-7">
                    <button className={"button is-success " + (mutation.isLoading ? "is-loading" : "")} type="submit" form="edit-webhook-form">{_('edit-form-save-button')}</button>
