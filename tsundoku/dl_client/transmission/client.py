...
class TransmissionClient(TorrentClient):
    ...
    async def check_torrent_completed(self, torrent_id: str) -> bool:
        resp = await self.request("torrent-get", {"ids": [torrent_id], "fields": ["isFinished", "status"]})
    ...
    async def check_torrent_ratio(self, torrent_id: str) -> float | None:
        resp = await self.request("torrent-get", {"ids": [torrent_id], "fields": ["uploadRatio"]})
    ...
    async def get_torrent_fp(self, torrent_id: str) -> Path | None:
        resp = await self.request("torrent-get", {"ids": [torrent_id], "fields": ["downloadDir", "name"]})
    ...
    async def request(self, method: str, arguments: dict | None = None) -> dict:
        ...
        while retries:
            ...
            async with self.session.post(request_url, json=body, headers=headers) as resp:
                ...
                if resp.status == 409:
                    retries -= 1
                    logger.warning(f"Transmission - Invalid session ID, retrying {retries}")
